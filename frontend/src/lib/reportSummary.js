...
// Template-based draft, not an LLM call: InfraScan has no OpenAI (or similar) API key wired up,
...
export function buildReportSummary(observation, jurisdiction) {
  ...
  return [
    ...
    '',
    `A ${observation.severity} severity ${type.toLowerCase()} was detected on ${jurisdiction.road_name} on ${capturedAt} at coordinates ${coords}, via InfraScan's automated road-defect scanning.`,
    '',
    `Tender reference ${jurisdiction.tender_number}, contractor of record: ${jurisdiction.contractor_name}. ${dlpNote}`,
    '',
    'Requesting inspection and repair at the earliest. Photographic evidence is attached/available via the InfraScan report link.',
  ].join('\n')
}
...
export function buildTweetSummary(observation, jurisdiction) {
  const type = capitalize(observation.defect_type?.replace('_', ' ')) || 'Defect'
  return `${type} (${observation.severity} severity) on ${jurisdiction.road_name} — tender ${jurisdiction.tender_number}, contractor ${jurisdiction.contractor_name}. Reported via InfraScan.`
}
